ReportGenerator.configure do
  section :hourly_ework_seed_active_aid do
  ...
  section :hourly_ework_seed_executed_aid do
    data_source DailyDs::TPhonearg
    distinct_count :uid
    condition ["t_phoneargs.first_time  between ? and ? " , params(:date).beginning_of_day, params(:date).end_of_day]
  ...
  section :hourly_ework_conversion do
  	data_source DailyDs::TPhonearg
  	count :uid
  	sum :url_success
  	sum :down_success
  ...
  report :hourly_ework_conversion do
