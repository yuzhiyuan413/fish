ReportGenerator.configure do
  ...
  section :ework_cross_solution_event_active do
    data_source EworkDs::TPhonearg
    distinct_count :uid
  ...
  section :ework_cross_solution_event_executing do
    data_source EworkDs::TPhonearg
    distinct_count :uid
    condition ["ework_production.t_phoneargs.first_time  between ? and ? " , params(:date).beginning_of_day, params(:date).end_of_day]
    condition ["sword_production.seed_feedback_histories.record_time > ?", params(:date).beginning_of_day]
  ...
  section :ework_cross_solution_event_executied do
    data_source EworkDs::TPhonearg
    distinct_count :uid
    condition ["ework_production.t_phoneargs.first_time  between ? and ? " , params(:date).beginning_of_day, params(:date).end_of_day]
    condition ["sword_production.seed_feedback_histories.record_time > ?", params(:date).beginning_of_day]
  ...
  section :ework_cross_solution_event_successed do
    data_source EworkDs::TPhonearg
    distinct_count :uid
    condition ["ework_production.t_phoneargs.first_time  between ? and ? " , params(:date).beginning_of_day, params(:date).end_of_day]
    condition ["sword_production.seed_feedback_histories.record_time > ?", params(:date).beginning_of_day]
  ...
  section :ework_cross_solution_event_installed do
    data_source EworkDs::TPhonearg
    distinct_count :uid
    condition ["ework_production.t_phoneargs.first_time  between ? and ? " , params(:date).beginning_of_day, params(:date).end_of_day]
    condition ["sword_production.seed_feedback_histories.record_time > ?", params(:date).beginning_of_day]
  ...
  section :ework_cross_solution_event_started do
    data_source EworkDs::TPhonearg
    distinct_count :uid
    condition ["ework_production.t_phoneargs.first_time  between ? and ? " , params(:date).beginning_of_day, params(:date).end_of_day]
    condition ["sword_production.seed_feedback_histories.record_time > ?", params(:date).beginning_of_day]
  ...
  section :ework_cross_solution_event_failed do
    data_source EworkDs::TPhonearg
    distinct_count :uid
    condition ["ework_production.t_phoneargs.first_time  between ? and ? " , params(:date).beginning_of_day, params(:date).end_of_day]
    condition ["sword_production.seed_feedback_histories.record_time > ?", params(:date).beginning_of_day]
  ...
  section :ework_cross_solution_event_V8 do
    data_source EworkDs::TPhonearg
    distinct_count :uid
    condition ["ework_production.t_phoneargs.first_time  between ? and ? " , params(:date).beginning_of_day, params(:date).end_of_day]
    condition ["sword_production.seed_feedback_histories.record_time > ?", params(:date).beginning_of_day]
  ...
  section :ework_cross_solution_event_V9 do
    data_source EworkDs::TPhonearg
    distinct_count :uid
    condition ["ework_production.t_phoneargs.first_time  between ? and ? " , params(:date).beginning_of_day, params(:date).end_of_day]
    condition ["sword_production.seed_feedback_histories.record_time > ?", params(:date).beginning_of_day]
  ...
  section :ework_cross_v9_activity do
    data_source EworkDs::TPhonearg
    distinct_count :uid
    condition ["ework_production.t_phoneargs.first_time  between ? and ? " , params(:date).beginning_of_day, params(:date).end_of_day]
    condition ["smart_v9_production.request_histories.server_time > ?", params(:date).beginning_of_day]
    condition ["substring_index(smart_v9_production.request_histories.loader_version_short,'.',1) = '9'"]
    join "inner join smart_v9_production.request_histories on smart_v9_production.request_histories.aid = ework_production.t_phoneargs.uid"
    group :tag
    sum_group :tag, Distributers::TagDistributer
  end

  report :ework_arrival do
    ...
    field :ework_activation  do
      section :ework_activation
      columns :tag, :count_distinct_uid
    end

    field :seed_active  do
      section :ework_cross_solution_event_active
      columns :tag, :count_distinct_uid
    end

    field :seed_executing  do
      section :ework_cross_solution_event_executing
      columns :tag, :count_distinct_uid
    end

    field :seed_executied  do
      section :ework_cross_solution_event_executied
      columns :tag, :count_distinct_uid
    end

    field :seed_successed  do
      section :ework_cross_solution_event_successed
      columns :tag, :count_distinct_uid
    end

    field :seed_installed  do
      section :ework_cross_solution_event_installed
      columns :tag, :count_distinct_uid
    end

    field :seed_started  do
      section :ework_cross_solution_event_started
      columns :tag, :count_distinct_uid
    end

    field :seed_failed  do
      section :ework_cross_solution_event_failed
      columns :tag, :count_distinct_uid
    end

    field :seed_v8_exists  do
      section :ework_cross_solution_event_V8
      columns :tag, :count_distinct_uid
    end

    field :seed_v9_exists  do
      section :ework_cross_solution_event_V9
      columns :tag, :count_distinct_uid
    end

    field :v9_activity  do
      section :ework_cross_v9_activity
      columns :tag, :count_distinct_uid
    end
